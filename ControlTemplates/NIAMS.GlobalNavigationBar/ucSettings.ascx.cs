...
namespace NIAMS.GlobalNavigationBar.ControlTemplates.NIAMS.GlobalNavigationBar
{
    public partial class ucSettings : UserControl
    {
        ...
        private void createTrvGroups()
        {
            try
            {
                ...
                SPSecurity.RunWithElevatedPrivileges(
                    delegate()
                    {
                        XDocument xDoc = XDocument.Load(MapPath(XMLGROUPNAMESPATH));
        ...
        private List<string> getIcons(string SiteUrl)
        {
            ...
            try
            {
                StringDictionary strdSettings = clsCommonBL.getSettings(SiteUrl, clsCommonBL.FindBy.BySiteUrl);
        ...
        private void fillPosition()
        {
            ...
            try
            {
                XDocument xDoc = XDocument.Load(this.MapPath(XMLGLOBALNAVPATH));
                string strTitle;
        ...
        private TreeNode createTreeViewNode(string strUrl, string strTitle, string strToolTip, TreeNode mniParent, List<string> strIcons)
        {
            ...
            try
            {
                ...
                {
                    strTitle += string.Format("<span></span><img height='15' align='middle' src='{0}/_layouts/NIAMS.GlobalNavigationBar/Images/{1}' alt=''/>", SPContext.Current.Web.Url, strIcon);
                }

                StringDictionary strdSettings = clsCommonBL.getSettings(strUrl, clsCommonBL.FindBy.BySiteUrl);
                ...
                if (strdSettings != null)
                {
                    if (Convert.ToInt16(String.IsNullOrEmpty(strdSettings["position"].ToString()) ? "0" : strdSettings["position"].ToString()) > 0)
                    {
                        ...
                        strTitle += string.Format("<span style='position: absolute;float:right;'></span><img height='15' align='middle' src='{0}/_layouts/NIAMS.GlobalNavigationBar/Images/{1}.jpg' alt=''></img>", SPContext.Current.Web.Url, strdSettings["position"]);
                    }
                }
        ...
        private void getSelectedNodeInfo(TreeNode e)
        {
            try
            {
                ...
                StringDictionary strdSettings = clsCommonBL.getSettings(e.Value, clsCommonBL.FindBy.BySiteUrl);
        ...
        protected void addNewGroupNameToXMLFile(string strGroupName)
        {
            try
            {
                ...
                {
                    string strPath = @"/_layouts/NIAMS.GlobalNavAdmnSettings/GroupNames.xml";
                    int GroupCount = 1;
                    XDocument xDoc = XDocument.Load(this.MapPath(strPath));
            ...
            try
            {
                SPSecurity.RunWithElevatedPrivileges(
                    delegate()
                    {
                        using (SPWeb web = SPContext.Current.Site.WebApplication.Sites[SiteUrl].OpenWeb())
                        {
                            //string strPath = @"/_layouts/NIAMS.GlobalNavigationAndFooterAdmnSettings/GlobalNav.xml";
                            XDocument xDoc = XDocument.Load(this.MapPath(XMLGLOBALNAVPATH));
        ...
        private void getSelectedNodeGroupInfo(TreeNode e)
        {
            try
            {
                StringDictionary strdSettings = clsCommonBL.getGroupSettings(e.Value);
