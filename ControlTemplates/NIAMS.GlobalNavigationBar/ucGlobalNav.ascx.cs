            ...
            try
            {
                ...
                SPSecurity.RunWithElevatedPrivileges(
                    delegate()
                    {
                        ...
                        XDocument xdMenuItems = XDocument.Load(MapPath(XMLGLOBALNAVPATH));
                        ...
                        if (iGlobalNavItemsCount > 1)
                        {
                            ...
                            foreach (XElement item in q.ToArray())
                            {
                                ...
                                {
                                    StringDictionary lstSettings = clsCommonBL.getSettings(item.Element("SiteUrl").Value, clsCommonBL.FindBy.BySiteUrl);
                ...
                if (SPContext.Current.Site.RootWeb.CurrentUser.IsSiteAdmin && Convert.ToBoolean(AddUrl))
                {
                    ...
                    btnSettings.ImageUrl = "/_layouts/NIAMS.GlobalNavigationBar/Images/Menu/tools.png";
                    btnSettings.ID = "lmgbtnSettings1";
                    btnSettings.Attributes.Add("onClick", "return openDialogModal('/_layouts/Niams.GlobalNavigationBar/settings.aspx', 'Settings')");
                ...
                SPSecurity.RunWithElevatedPrivileges(
                    delegate()
                    {
                        ...
                        if (iGlobalNavItemsCount > 1)
                        {
                            ...
                            if (!String.IsNullOrEmpty(strGroupParentId))
                            {
                                XDocument xGroups = XDocument.Load(MapPath(XMLGROUPNAMESPATH));
                            ...
                            else
                            {
                                var q = from c in xDoc.Elements("GlobalNav").Elements("Item")
                                        where (string.IsNullOrEmpty(c.Element("ParentId").Value.ToString()) || (clsCommonBL.ParentExist(c.Element("ParentId").Value.ToString()) == 0)) &&
            ...
            try
            {
                ...
                foreach (var item in q)
                {
                    if (clsCommonBL.IsUserHasAccess(item.Element("SiteUrl").Value, userLoginName))
                    {
                        ...
                        StringDictionary strdChildSettings = clsCommonBL.getSettings(item.Element("SiteId").Value, clsCommonBL.FindBy.BySiteId);
            ...
            try
            {
                ...
                if (Children != null && Children.Count() > 0)
                {
                    ...
                    foreach (var Child in Children)
                    {
                        ...
                        {
                            StringDictionary strdChildSettings = clsCommonBL.getSettings(Child.Element("SiteId").Value, clsCommonBL.FindBy.BySiteId);
